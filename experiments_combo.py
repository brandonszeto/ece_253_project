...
def apply_grayscale_morph_clahe_inseq(image):
    """Apply grayscale and morphological closing w/ elliptical kernel and clahe"""
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY) if image.ndim == 3 else image

    # Automatically choose a large kernel based on image size
    h, w = gray.shape
    k = int(0.1 * min(h, w))     # ~10% of smaller image dimension
    if k % 2 == 0:
        k += 1   # kernel size must be odd
    k = max(k,31)
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (k, k))

    # Morphological closing (background / illumination estimate)
    background = cv2.morphologyEx(gray, cv2.MORPH_CLOSE, kernel)
    background = np.clip(background, 1, 255) # Avoid division by zero
    corrected = cv2.divide(gray, background, scale=255)     # Illumination correction (division)
    corrected = corrected.astype(np.uint8)
    
    # CLAHE in grayscale
    clahe = cv2.createCLAHE(clipLimit = 2.0, tileGridSize=(8,8))
    corrected_clahe = clahe.apply(corrected)
    
    # Convert to LAB colorscale
    lab = cv2.cvtColor(image, cv2.COLOR_BGR2LAB)
    lab[:,:,0] = corrected_clahe
...
def main():
    ...
    techniques = [
        ...
        ("combo", apply_grayscale_morph_clahe_inseq)
    ]
